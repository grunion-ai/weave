...
test('every registered name draws, and the registry is the whole vendored set', () => {
  assert.ok(reg.NAMES.length >= 555, `movingicons.dev ships 555; got ${reg.NAMES.length}`);
...
test('most of the set moves — motion is the point of the switch', () => {
  ...
  assert.ok(moving.length >= 500, `only ${moving.length} icons move`);
...
test('the motion CSS is scoped per icon and nothing loops', () => {
  ...
  assert.ok(selectors.length > 500, 'the motion rules ride along');
