...
test('weaver seed: docs, wiki, quality mirror, issues + roadmap', () => {
  ...
  assert.equal(w.state.meta.name, 'weaver');
...
test('multi-workspace hub: scoped routing, listing, cross-workspace search', async () => {
  ...
  const weaver = seedWeaver(new Weave());

  const { server } = await startServer(uno, { port: 0, workspaces: { weaver } });
  const base = `http://127.0.0.1:${server.address().port}`;
  try {
    // Workspace list
    const list = await (await fetch(`${base}/api/workspaces`)).json();
    assert.deepEqual(list.map((w) => w.name).sort(), ['uno', 'weaver']);
    ...
    const wHealth = await (await fetch(`${base}/w/weaver/api/health`)).json();
    assert.equal(wHealth.workspace, 'weaver');
    const guides = await (await fetch(`${base}/w/weaver/api/tables/Guide/query`, {
    ...
    const docHtml = await (await fetch(`${base}/w/weaver/e/${guideId}/doc.html`)).text();
    assert.match(docHtml, /<h1>/);
    const redirect = await fetch(`${base}/w/weaver/e/${guideId}`, { redirect: 'manual' });
    assert.equal(redirect.status, 302);
    assert.match(redirect.headers.get('location'), /^\/w\/weaver\/#\/entity\//);

    // Unknown workspace 404s
    assert.equal((await fetch(`${base}/w/nope/api/health`)).status, 404);

    // Scoped search prefixes permalinks
    const scoped = await (await fetch(`${base}/w/weaver/api/search?q=quickstart`)).json();
    assert.ok(scoped.some((h) => h.url.startsWith('/w/weaver/')));
    ...
    assert.ok(allDocs.some((h) => h.workspace === 'weaver'));

    // Static UI served under the workspace path too
    const page = await (await fetch(`${base}/w/weaver/`)).text();
    assert.match(page, /<title>Weave<\/title>/);

    // .mmd document format
    const mmd = await fetch(`${base}/w/weaver/e/${guideId}/doc.mmd`);
