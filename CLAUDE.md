...
- TDD: `npm test` (run from this directory) must be green before any commit; new engine/server behavior lands with tests. It goes through `scripts/test.mjs`, which runs the files that failed a second time and lets that answer stand — a browser case that loses a paint to a 30 s budget cost a Verified −1 on a good change roughly every other run (Issue #44), and a real regression fails both times. Every retried file is named under `# RETRYING` / `# RETRIED`, which is inside the tail the gate quotes into its vote: a suite that keeps needing the second chance gets an Issue, not a shrug. `node --test 'test/**/*.test.mjs'` still works and has no retry.
